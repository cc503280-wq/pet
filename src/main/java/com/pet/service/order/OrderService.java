	...
	public Order userOrder(OrderCheckOutDTO orderCheckOutDTO) {
		...
		if (orderCheckOutDTO.getCouponId() != null) {
			...
			if ("amount".equals(coupon.getDiscountType())) {
				// 固定金額折抵（例如折 100 元）
				amountAfterDiscount = totalAmount.subtract(discountValue);

			} else if ("rate".equals(coupon.getDiscountType())) {
				// 比例折扣（例如 0.9 = 打 9 折）
				amountAfterDiscount = totalAmount.multiply(discountValue);
			}
			// 防止折到負數
		...
		// 計算滿百贈點(無條件捨去)
		Integer getPoints = totalAmountDiscountPoints.divide(BigDecimal.valueOf(100), 0, java.math.RoundingMode.FLOOR)
				.intValue();
		// 2.建立並儲存訂單主表
		...
		if (orderCheckOutDTO.getCouponUsersId() != null) {
			couponUsersRealRepository.UpdateCouponUsersReal(orderCheckOutDTO.getCouponUsersId(), "used",
					LocalDate.now());
		}
